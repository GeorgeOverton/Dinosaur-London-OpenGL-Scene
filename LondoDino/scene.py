...
class Scene:
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.move == True:
                # activates when someone clicks space
                continousTime = continousTime + dt
                self.update(continousTime / 1000.0)  # Convert milliseconds to seconds
            self.draw()
    
    def update(self, continousTime):
        #rotates thje dino
        rotationSpeed = 4.5
        angle = continousTime * rotationSpeed
        self.table = [DrawModelFromMesh(scene=self, M=np.matmul(translationMatrix([0, -4, 0]),rotationMatrixY(angle)), mesh=mesh, shader=ShadowMappingShader(shadow_map=self.shadows), name='table') for mesh in self.table1]
