...
def process_line(line):
	...
	#comment line in obj file
	if fields[0] == '#':
		label = 'comment'
		return (label, fields[1:])

	#contians description of a vetex
	elif fields[0] == 'v':
		label = 'vertex'
		if len(fields) != 4:
	...
	#contains the co ordinate of the texture for the vertex
	elif fields[0] == 'vt':
	...
	# used to identify a meterial that a object uses
	...
	# used to identify which vetacies are a part of a face
	elif fields[0] == 'f':
		label = 'face'
...
def load_material_library(file_name):
	...
	#opens the mtl file to be read
	mtlfile = open(file_name)
	for line in mtlfile:
		fields = line.split()
		if len(fields) != 0:
			#loads in the features from the mtl file by marking where the texture is applied and the image if needed
			if fields[0] == 'newmtl':
...
def load_obj_file(file_name):
	...
	with open(file_name) as objfile:
		...
		for line in objfile:
			...
			# adds all vertacies from the obj file to a list
			elif data[0] == 'vertex':
				vlist.append(data[1])
			...
			# adds all vertacex textures from the obj file to a list
			elif data[0] == 'vertex texture':
				tlist.append(data[1])

			# adds all faces from the obj file to a list
			elif data[0] == 'face':
				if len(data[1]) == 3:
					flist.append(data[1])
	...
	v_array = np.array(vlist, dtype=np.float32)
	f_array = np.array(flist, dtype=np.int32)

	return create_meshes_from_blender(v_array, f_array, mlist, tlist, library, mesh_list, lnlist)
